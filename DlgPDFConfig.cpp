...
IMPLEMENT_DYNAMIC(CDlgPDFConfig, CDialog)

CDlgPDFConfig::CDlgPDFConfig(CWnd* pParent /*=NULL*/)
	: CDialog(CDlgPDFConfig::IDD, pParent)
{
	m_iType = -1;
...
void CDlgPDFConfig::DoDataExchange(CDataExchange* pDX)
{
	CDialog::DoDataExchange(pDX);
}


BEGIN_MESSAGE_MAP(CDlgPDFConfig, CDialog)
	ON_BN_CLICKED(IDNEXT, &CDlgPDFConfig::OnBnClickedNext)
	ON_BN_CLICKED(IDCANCEL, &CDlgPDFConfig::OnBnClickedCancel)
	ON_BN_CLICKED(IDC_WINDCHILL_PROCRULE_BTN_ADD, &CDlgPDFConfig::OnBnClickedWINDCHILLProcruleBtnAdd)
	ON_BN_CLICKED(IDC_WINDCHILL_PROCRULE_BTN_DELETE, &CDlgPDFConfig::OnBnClickedWINDCHILLProcruleBtnDelete)
	ON_BN_CLICKED(IDC_WINDCHILL_PROCRULE_BTN_MOVEUP, &CDlgPDFConfig::OnBnClickedWINDCHILLProcruleBtnMoveup)
	ON_BN_CLICKED(IDC_WINDCHILL_PROCRULE_BTN_MOVEDOWN, &CDlgPDFConfig::OnBnClickedWINDCHILLProcruleBtnMovedown)
	ON_REGISTERED_MESSAGE(BCGM_GRID_ITEM_CHANGED, OnItemChanged)
	ON_REGISTERED_MESSAGE(BCGM_GRID_SEL_CHANGED, OnSelChanged)
	ON_WM_SIZE()
	ON_BN_CLICKED(IDLAST, &CDlgPDFConfig::OnBnClickedLast)
END_MESSAGE_MAP()


BOOL CDlgPDFConfig::OnInitDialog()
{
	...
	if (!flag)
	{
		MessageBox(_T("模板文件有误，请确认！"), APS_MSGBOX_TITIE, MB_OK);
		EndDialog(S_FALSE);
		return FALSE;
	}
...
bool CDlgPDFConfig::ParseConfigFile(CString strxml, CStringArray & PDFTempLateProcArray, CStringArray & PDFTempLateStepArray)
{
...
void CDlgPDFConfig::GetStepTemp(CStringArray &StrStepTemp)
{
	CString strIni = GetModuleDir() + _T("\\PDFTemplatePlan.ini");
	TCHAR chSectionnames[2048] = { 0 };
	char * pSectionName;
...
// CDlgPDFConfig 消息处理程序


void CDlgPDFConfig::OnBnClickedNext()
{
	SaveConfig();
...
void CDlgPDFConfig::OnBnClickedCancel()
{
	m_iMode = 0;
...
void CDlgPDFConfig::OnBnClickedWINDCHILLProcruleBtnAdd()
{
...
void CDlgPDFConfig::OnBnClickedWINDCHILLProcruleBtnDelete()
{
...
void CDlgPDFConfig::OnBnClickedWINDCHILLProcruleBtnMoveup()
{
	bool bRedraw = 1;
...
void CDlgPDFConfig::OnBnClickedWINDCHILLProcruleBtnMovedown()
{
	bool bRedraw = 1;
	CBCGPGridRow* pCurRow = NULL;
	int iMax = m_ProcList->GetRowCount(0) - 1;
...
BOOL CDlgPDFConfig::PreTranslateMessage(MSG* pMsg)
{
...
LRESULT CDlgPDFConfig::OnSelChanged(WPARAM wp, LPARAM lp)
{
...
LRESULT CDlgPDFConfig::OnItemChanged(WPARAM wp, LPARAM lp)
{
	BCGPGRID_ITEM_INFO * iteminfo = (BCGPGRID_ITEM_INFO*)lp;
...
LRESULT CDlgPDFConfig::OnItemDblClk(WPARAM wp, LPARAM lp)
{
	return 0;
}

void CDlgPDFConfig::OnSize(UINT nType, int cx, int cy)
{
	CRect rc1, parrect;
	GetClientRect(&parrect);
...
void CDlgPDFConfig::setTemplateName(CString sName)
{
	m_sTemplateName = sName;
}
void CDlgPDFConfig::setTemplateType(int iType)
{
	m_iType = iType;
}
void CDlgPDFConfig::SetProcess(CProcess *pProcess)
{
	m_pProcess = pProcess;
}

void CDlgPDFConfig::SaveConfig()
{
	//保存工序类模板的配置
...
int CDlgPDFConfig::getMode()
{
	return m_iMode;
}

void CDlgPDFConfig::OnBnClickedLast()
{
	m_iMode = 2;
