...
CString CWebServiceInterface::getDocInfoOfContent(CString  operType, CString docNumber)
{
	...
	ns1__getDocInfoOfContent trans;
...
BOOL ExeCuteFTPRquestCommandByQuote(CURL *curl,char *pURL,unsigned short usPort,char *pUsername,char *pPassword,char*pCommand,BOOL bIsGetResponseData,std::string &strResponse,int nConnectimeout=30,int nTimeout=30)
{
	...
	curl_easy_setopt(curl,CURLOPT_CONNECTTIMEOUT,nConnectimeout);
...
BOOL CFTPInterface::Delete(CString localfile, CString ftpurl, CString ftpport, CString relative_dir, CString filename, CString strUserName, CString strPasswd)
{
	...
	char fullpath[512];
	memset(fullpath, 0, 512);
	sprintf(fullpath, "%s%s", ftpurl, dir);
	...
	curl_easy_setopt(m_pCurl,CURLOPT_PORT,atoi(ftpport.GetBuffer()));
	...
	curl_easy_setopt(m_pCurl,CURLOPT_CONNECTTIMEOUT,30);
...
#if 1
	auto dir = relative_dir.Right(relative_dir.GetLength()-1);
	char szComand[200]={0};
	sprintf_s(szComand,"CWD %s",dir);
	std::string strResponse ="";
	auto flgchangdir =ExeCuteFTPRquestCommandByQuote(m_pCurl,ftpurl.GetBuffer(),21,strUserName.GetBuffer(),strPasswd.GetBuffer(),szComand,false,strResponse);
	char szComand1[200]={0};
	sprintf_s(szComand1,"DELE %s",filename);
	auto  flagdelete =ExeCuteFTPRquestCommandByQuote(m_pCurl,ftpurl.GetBuffer(),21,strUserName.GetBuffer(),strPasswd.GetBuffer(),szComand1,false,strResponse);
	if (flgchangdir&&flagdelete)
	{
		return 1;
	}
	else 
